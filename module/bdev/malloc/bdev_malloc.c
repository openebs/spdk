...
malloc_disk_resize_cb(struct spdk_bdev *bdev, void *cb_arg, int status)
{
	...
	SPDK_NOTICELOG("++++^ MALLOC RESIZED %s: -> %lu Mb\n",
		    bdev->name,
		    bdev->blocklen * bdev->blockcnt / (1024 * 1024));
}
...
resize_malloc_disk(const char *bdev_name, uint64_t new_size_in_mb)
{
	...
	SPDK_ERRLOG("++++ MALLOC WILL RESIZE %s: %lu Mb -> %lu Mb\n",
		    bdev->name,
		    current_size_in_mb, new_size_in_mb);
