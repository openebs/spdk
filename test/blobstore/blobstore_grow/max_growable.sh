...
if [ "$SYSTEM" = "FreeBSD" ]; then
	echo "max_growable.sh cannot run on FreeBSD currently."
	exit 0
fi
...
function create_lvs() {
	...
	$rpc_py bdev_aio_create "$aio_disk" "$bdev_name" 512
...
function grow_lvs() {
	...
	growable_upto=$(./scripts/rpc.py bdev_lvol_get_lvstores -l "$lvs_name" | jq -r '.[0].max_growable_size')
	num_blocks=$(./scripts/rpc.py bdev_get_bdevs -b "$bdev_name" | jq -r '.[0].num_blocks')
	current_size=$((num_blocks * 512))
	...
	$rpc_py bdev_aio_rescan "$bdev_name"
	expect_fail "$rpc_py" bdev_lvol_grow_lvstore -l "$lvs_name"
	# Cleanup lvs, bdev and disk.
	./scripts/rpc.py bdev_lvol_delete_lvstore -l "$lvs_name"
	./scripts/rpc.py bdev_aio_delete "$bdev_name"
	rm -f "$aio_disk"
}

expect_fail() {
	if "$@"; then
		echo "FAIL: Grow was not expected to succeed"
		exit 1
	else
		echo "PASS: Grow expected to fail beyond max growable size"
	fi
}
...
# LVS backed by 300GiB device, 16iB cluster size and 300 mum_md_pages_cluster_ratio.
create_lvs $((300 * 1024 * 1024 * 1024)) $((16 * 1024 * 1024)) 300
grow_lvs

# LVS backed by 500GiB device, 12iB cluster size and 600 mum_md_pages_cluster_ratio.
create_lvs $((500 * 1024 * 1024 * 1024)) $((12 * 1024 * 1024)) 600
grow_lvs

# LVS backed by 1024GiB device, 254GiB cluster size and 800 mum_md_pages_cluster_ratio.
